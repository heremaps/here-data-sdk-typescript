...
/**
 * @internal
 * Implementation of `UploadRequest` for the Blob V2.
 */
export class BlobV2UploadRequest implements UploadRequest {
    ...
    async uploadPart(opts: {
        ...
        if (!result.id) {
            return Promise.reject(
                new Error(
                    `Error uploading chunk ${opts.partNumber}, can not get the part ID from the response`
                )
            );
        }
