...
describe("CatalogsRequest", function () {
    ...
    it("Should set parameters", function () {
        const catalogsRequest = new CatalogsRequest();
        const catalogsRequestWithSchemaHrn =
            catalogsRequest.withSchema(mockedSchemaHRN1);
        const catalogsRequestWithBillTag =
            catalogsRequest.withBillingTag(billingTag);
