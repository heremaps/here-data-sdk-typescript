...
describe("VolatileLayerClient", function() {
  ...
  it("Shoud read partitions metadata by QuadKey for specific VolatileLayer", async function() {
    ...
    mockedResponses.set(
      `https://query.data.api.platform.here.com/query/v1/layers/test-layed-id/quadkeys/012/depths/3`,
      new Response(
        JSON.stringify({
          parentQuads: [
            {
  ...
  it("Shoud read partitions with additionalFields parameter from QuadKeyPartitionsRequest", async function() {
    ...
    // Set the response of mocked partitions with additional fields.
    mockedResponses.set(
      `https://query.data.api.platform.here.com/query/v1/layers/test-layed-id/quadkeys/012/depths/0?additionalFields=dataSize,checksum,compressedDataSize`,
      new Response(JSON.stringify(mockedPartitions), { headers })
    );
  ...
  it("GetAggregatedData", async function() {
        ...
        JSON.stringify([
          {
            ...
            version: "v1",
            baseURL:
              "https://volatile-blob.data.api.platform.here.com/hrn:here:data:::test-hrn/v1",
            parameters: {
              additionalProp1: "string",
              additionalProp2: "string",
              additionalProp3: "string"
    ...
    mockedResponses.set(
      `https://query.data.api.platform.here.com/hrn:here:data:::test-hrn/v1/layers/mocked-layer/quadkeys/1331132/depths/4`,
      new Response(
    ...
    mockedResponses.set(
      `https://volatile-blob.data.api.platform.here.com/hrn:here:data:::test-hrn/v1/layers/mocked-layer/data/5D220F614ED66FDD4850E9846482193A.309`,
      new Response(Buffer.alloc(100))
    );

    mockedResponses.set(
      `https://volatile-blob.data.api.platform.here.com/hrn:here:data:::test-hrn/v1/layers/mocked-layer/data/436735897B05C80C449B1D654541DE5E.309`,
