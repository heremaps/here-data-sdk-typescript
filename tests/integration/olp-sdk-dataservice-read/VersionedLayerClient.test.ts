...
describe("VersionedLayerClient", function () {
    ...
    it("Shoud be initialised with VersionedLayerClientParams without version", async function () {
        ...
        expect(layerClientWithoutVersion).to.be.instanceOf(
            VersionedLayerClient
        );
    });
    ...
    it("Shoud be fetched data with dataHandle", async function () {
        ...
        expect(fetchStub.mock.calls.length).to.be.equal(2); // 1 - lookup, 1 - blob
    });
    ...
    it("Shoud fetch partitions with additionalFields if cached the same partitions, but without additionalFields", async function () {
        ...
        expect(partitions.partitions[1].compressedDataSize).to.be.equal(
            mockedPartitionsWithAdditionalFields.partitions[1]
                .compressedDataSize
        );
        expect(partitions.partitions[1].dataSize).to.be.equal(
