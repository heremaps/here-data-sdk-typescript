...
describe("VersionedLayerClient", function() {
  ...
  it("Shoud read partitions metadata by QuadKey for specific VersionLayer", async function() {
    ...
    mockedResponses.set(
      `https://query.data.api.platform.here.com/query/v1/layers/test-layed-id/versions/42/quadkeys/012/depths/3`,
      new Response(
        JSON.stringify({
          parentQuads: [
            {
  ...
  it("Shoud read partitions with additionalFields parameter from QuadKeyPartitionsRequest", async function() {
    ...
    // Set the response of mocked partitions with additional fields.
    mockedResponses.set(
      `https://query.data.api.platform.here.com/query/v1/layers/test-layed-id/versions/30/quadkeys/012/depths/0?additionalFields=dataSize,checksum,compressedDataSize`,
      new Response(JSON.stringify(mockedPartitions), { headers })
    );
  ...
  it("GetAggregatedData", async function() {
    ...
    mockedResponses.set(
      `https://query.data.api.platform.here.com/hrn:here:data:::test-hrn/v1/layers/mocked-layer/versions/123/quadkeys/1331132/depths/4`,
      new Response(
