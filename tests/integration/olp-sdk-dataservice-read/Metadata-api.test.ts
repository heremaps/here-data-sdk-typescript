...
describe("CatalogClient", function () {
    ...
    it("Should be passsed parameter Range to the headers of the metadata request", async function () {
        ...
        const callsToApi = fetchStub.mock.calls;
        ...
        expect(callToMetadata[1].headers.get("Authorization")).equals(
            "Bearer test-token-string"
        );
        expect(callToMetadata[1].headers.get("Range")).equals("bytes=10-");
    });
});
