...
describe("StreamLayerClient", function () {
    ...
    it("Should getData() method handle error", async function () {
        ...
        const data = await streamClient
            .getData({} as Message)
            .catch((error) => {
                expect(error.message).to.be.equal(
                    "No data handle for this partition"
                );
            });
    });
    ...
    it("Shoult unsubscribe() method cancel subscription", async function () {
        ...
        const subscribtionId = await streamClient.subscribe(
            new SubscribeRequest()
        );
        const request = new UnsubscribeRequest().withSubscriptionId(
            subscribtionId
        );
    ...
    it("Should unsubscribe() method handle errors", async function () {
        ...
        const messages1 = await streamClient
            .unsubscribe(request1)
            .catch((error) => {
                expect(error.message).to.be.equal(
                    "Error: for 'parallel' mode 'subscriptionId' is required."
                );
